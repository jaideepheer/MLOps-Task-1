...
def test_splits():
    # create splits fo N=100
    X_train, X_test, X_valid, Y_train, Y_test, Y_valid = get_splits(
        0.2, 0.1, 100)
    # test case for N=100
...
def test_model_writing(tmp_path: pathlib.Path):
    ...
    # }
    # write model
    plot.write_trained_model(model, tmp_path)
    # check for specific pattern of model files
    out_file = (
        tmp_path
        / utils.serialize_dict_to_filename(model)
        / "model.gz"
    )
...
def test_small_data_overfit_checking():
    ...
    f1_threshold = 0.5
    # test
    assert model["valid_metrics"]["acc"] > acc_threshold
    assert model["valid_metrics"]["f1"] > f1_threshold
